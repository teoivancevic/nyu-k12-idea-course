...
import { Menu, Home, Lock, Calendar } from "lucide-react"
...
export function DayNavMobile() {
  const pathname = usePathname()
  const [open, setOpen] = useState(false)
  const currentDay = courseConfig.schedule.find(
    (d) => pathname === `/day/${d.day}`,
  )

  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger className="flex items-center gap-2 rounded-lg border px-3 py-2 text-sm lg:hidden">
